...
export class TokenSwapScenario extends Scenario {
  ...
  public async init() {
    ...
    // let smartAccountBalance  = await this.env.test.erc20.balanceOf(this.env.simpleAccounts[0].contractAddress);
    let smartAccountBalance = await ethers.provider.getBalance(this.env.simpleAccounts[0].contractAddress);
    console.log("smart account balance before : ", smartAccountBalance)
  ...
  public async run(count?: string[] | number, options?: ScenarioOptions): Promise<ScenarioResult> {
    ...
    if (options.useCompression) {
      if (aggregatedSignature) {
        console.log("if condition handleIntentsAggregated");
        ...
        console.log("if condition handleIntents");
    ...
    // let smartAccountBalance  = await this.env.test.erc20.balanceOf(this.env.simpleAccounts[0].contractAddress);
    let smartAccountBalance = await ethers.provider.getBalance(this.env.simpleAccounts[0].contractAddress);
    console.log("smart account balance after : ", smartAccountBalance)
