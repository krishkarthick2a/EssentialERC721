...
export class TransferErc20Scenario extends Scenario {
  ...
  public async init() {
    ...
    for (const account of this.env.simpleAccounts) {
      // console.log("inside simple accounts"); //all are working
      // console.log("code : ", await ethers.provider.getCode(account.contractAddress));
    ...
    for (const account of this.env.blsAccounts) {
      // console.log("inside bls accounts");
      const needToMint = ethers.parseEther('1000') - (await this.env.test.erc20.balanceOf(account.contractAddress));
      if (needToMint > 0) {
        await (await this.env.test.erc20.mint(account.contractAddress, needToMint)).wait();
      }
    ...
    for (const account of this.env.eoaProxyAccounts) {
      // console.log("inside eoa proxy accounts");
  ...
  public async run(count?: string[] | number, options?: ScenarioOptions): Promise<ScenarioResult> {
    ...
    if (options.useCompression) {
      console.log("inside compression");
      ...
      if (aggregatedSignature) {
        console.log("inside transferErc20Scenario.ts -> if");
        ...
        console.log("inside transferErc20Scenario.ts -> else txPromise");
        ...
        // const mnemonic = ethers.entropyToMnemonic(randomBytes);

// Create a wallet using the generated mnemonic phrase
// const wallet = ethers.Wallet.fromMnemonic(mnemonic);
        var newAddr = await wallet.getAddress();
        console.log("new wallet : ", newAddr);
        txPromise = this.env.entrypoint.connect(firstSigner).handleIntents(solution);
      }
    }
    const tx = await txPromise;
