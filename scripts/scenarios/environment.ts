...
export async function deployTestEnvironment(
  ...
  const paymaster = "0x5FbDB2315678afecb367f032d93F642f64180aa3"; //added
  const entrypoint = await ethers.deployContract('EntryPoint', [], deployer);
  const entrypointAddress = await entrypoint.getAddress();
  await entrypoint.depositTo(entrypointAddress, {value: ethers.parseEther("1")});  //added
  console.log("entrypoint contract deployed at : ", entrypointAddress);
  ...
  console.log("solver address : ", solverUtilsAddress);
  console.log("balance of solver address : ", await ethers.provider.getBalance(solverUtilsAddress));
  console.log("inside environment deployTestEnvironment : ", solverUtilsAddress);
  ...
  console.log("inside environment simpleAccountFactory : ", simpleAccountFactory.target);
  ...
  for (let i = 0; i < config.numAccounts; i++) {
    ...
    // console.log("inside environment smart address : ", contractAddress);
  ...
  // console.log("inside environment, accounts created");
  ...
  for (let i = 0; i < config.numAccounts; i++) {
    ...
    console.log(`owner address : ${ownerAddress} , smart address : ${contractAddress}`);
