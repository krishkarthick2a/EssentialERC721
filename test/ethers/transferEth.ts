...
describe('Transfer ETH Test', () => {
  ...
  //     useBLSSignatureAggregation: false,
  //   };
  //   await runMultiTest(scenarioOptions); //original
  // });
  ...
  async function runSingleTest(scenarioOptions: ScenarioOptions) {
    console.log(".ts transferEth runSingleTest balance before transaction : ", 
      await ethers.provider.getBalance(await env.deployer.getAddress()));
    ...
    console.log(".ts transferEth runSingleTest balance after transaction : ", 
      await ethers.provider.getBalance(await env.deployer.getAddress()));
  }
